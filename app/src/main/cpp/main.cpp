...
Java_com_xiaohaoo_yolo_ui_activity_MainActivity_00024Companion_initialize(JNIEnv *env, jobject thiz, jobject context) {
    ...
    TfLiteInterpreterOptionsSetNumThreads(interpreter_options, 10);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_xiaohaoo_yolo_ui_activity_MainActivity_00024Companion_finalize(JNIEnv *env, jobject thiz) {
    if (interpreter) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_xiaohaoo_yolo_ui_activity_MainActivity_00024Companion_run(JNIEnv *env, jobject thiz, jobject input, jobject output) {
    TfLiteStatus status = TfLiteInterpreterAllocateTensors(interpreter);
    TfLiteTensor *input_tensor = TfLiteInterpreterGetInputTensor(interpreter, 0);
    TfLiteTensorCopyFromBuffer(input_tensor, env->GetDirectBufferAddress(input), env->GetDirectBufferCapacity(input));
    auto start = std::chrono::high_resolution_clock::now();
    TfLiteInterpreterInvoke(interpreter);
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    __android_log_print(ANDROID_LOG_INFO, TAG, "duration size: %lld", duration.count());
    TfLiteType tensor_type = TfLiteTensorType(input_tensor);
    const TfLiteTensor *output_tensor = TfLiteInterpreterGetOutputTensor(interpreter, 0);
    memcpy(env->GetDirectBufferAddress(output), TfLiteTensorData(output_tensor), env->GetDirectBufferCapacity(output));
}